class Car62 {
	...
	Car62(String aModel, int aYear, String aColor) {
		name = new CarName62(aModel, aYear);
		color = aColor;
	}

	class CarName62 { // 이너 클래스는 클래스가 아닌 객체에 소속되며, 따라서 주 클래스 외부에서
		String model; // 이너 클래스 객체를 생성하려면 31행과 같이 주 객체가 먼저 생성되어야만
		int year; // 주 클래스에 소속된 이너 클래스 타입의 객체 생성 가능.

		CarName62(String aModel, int aYear) {
			model = aModel;
			year = aYear;
		}
...
public class Main62 {
	public static void main(String[] args) {
		Car62 pride62 = new Car62("프라이드", 2005, "파랑");

		pride62.outInfo();
		pride62.name.outColor(); // 이너 클래스 내부 멤버는 주 클래스 객체 내부 생성된 이너 클래스 객체를 통해서만 접근 가능.
...
//      pride.CarName pride2 = pride.new CarName("프랑이",2009);
		...
		Car62.CarName62 pride2 = pride62.new CarName62("프랑이", 2009);

		System.out.println();

		pride62.outInfo(); // pride62는 외부에 생성된 이너 클래스 타입의 독립적인 객체로써
